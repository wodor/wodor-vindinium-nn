...
const SavedNNsPanel: React.FC<SavedNNsPanelProps> = ({
  ...
  return (
    <>
      <div className={`fixed right-0 top-[57px] h-[calc(100vh-57px)] z-40 transition-all duration-300 ${isExpanded ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          {isExpanded && (
            <>
              ...
              <div className="flex-1 overflow-y-auto p-4 no-scrollbar">
                {savedNNs.length === 0 ? (
                  ...
                  <SavedNNsWidget
                    ...
                    onLoadToTraining={onLoadToTraining}
              ...
              <button
                ...
                className="absolute top-0 right-0 w-12 h-12 bg-slate-900/95 backdrop-blur-xl border-l border-white/5 flex items-center justify-center text-slate-400 hover:text-white transition-colors z-50"
      ...
      {!isExpanded && (
        <button
          onClick={onToggleExpanded}
          className="fixed right-0 top-[57px] w-12 h-12 bg-slate-900/95 backdrop-blur-xl border-l border-white/5 flex items-center justify-center text-slate-400 hover:text-white transition-colors z-50 shadow-2xl"
          title="Expand panel"
        >
          <span className="text-lg">←</span>
        </button>
