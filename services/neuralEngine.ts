...
/**
 * Optimized Inference Engine with Peripheral Radar
 */
export class NeuralEngine {
  ...
  static async getInference(state: GameState, heroId: number, weights: ModelWeights): Promise<AIDecision> {
    ...
    // Group A: Local Vision (25 neurons)
    for (let dy = -2; dy <= 2; dy++) {
      for (let dx = -2; dx <= 2; dx++) {
        ...
        if (tx < 0 || tx >= size || ty < 0 || ty >= size) {
          ...
          else if (tile === '##') inputs[idx++] = -1;
          else if (tile === '[]') inputs[idx++] = 1;
          else if (tile.startsWith('$')) inputs[idx++] = 0.5;
          else if (tile.startsWith('@')) inputs[idx++] = -0.5;
          else inputs[idx++] = 0;
        }
      }
    ...
    const nearestEnemyMine = findNearest((t, p) => t.startsWith('$') && t !== '$-' && t !== `$${hero.id}`);
    const nearestHero = findNearest((t) => t.startsWith('@') && t !== `@${hero.id}`);
    ...
    inputs[idx++] = nearestHero[0];
    inputs[idx++] = nearestHero[1];

    // 2. Hidden Layer Calculation (Vectorized simulation)
    ...
    return {
      move: moveMap[maxIdx],
      reasoning: `Radar locks: Tavern(${nearestTavern[0].toFixed(1)},${nearestTavern[1].toFixed(1)}), Mine(${nearestNeutralMine[0].toFixed(1)},${nearestNeutralMine[1].toFixed(1)}). Activation cluster ${maxIdx} firing.`,
      confidence: 0.95,
      latency
    };
  }
  ...
  static mutateWeights(weights: ModelWeights, mutationRate: number): ModelWeights {
    const mutate = (matrix: number[][]) => 
      matrix.map(row => row.map(val => {
        if (Math.random() < mutationRate) {
           return val + (Math.random() * 2 - 1) * 0.15;
        }
        return val;
      }));
